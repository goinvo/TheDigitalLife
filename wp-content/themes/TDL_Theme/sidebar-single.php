			...
			<?php for ($i=1; $i<=4; $i++) { 
				...
				$bio = 'guest_bio_'. $i;
				if (get_field($bio) != null) {
				    echo '<style>.morecontent{display:none;}</style>';
                		    echo '<h6 class="bio more">'; 			    
			    	    echo the_field($bio);
			    	    echo '</h6>';
				}
			...
			?>
			    <div class="credits">
				<h2 class="epi-headings tk-adelle">Credits</h2>
				<?php the_field('credits'); ?>
			    </div>
			    <?php if (get_the_tags()) { ?>
			    <div class="tags">
				<h2 class="epi-headings tk-adelle">Tags</h2>
			    	<?php the_tags('', ' '); ?> 
			    </div>  
			    <?php } ?>